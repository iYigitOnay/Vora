            ...
            <motion.div key="s1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
              <div className="space-y-8">
                <div className="relative group">
                  <label className="text-[9px] font-black text-vora-tertiary uppercase tracking-[0.4em] mb-2 block opacity-30 group-focus-within:text-vora-accent transition-colors">Besin Adı</label>
                  <input autoFocus type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value.toUpperCase()})} onKeyDown={(e) => e.key === 'Enter' && handleNextStep()} placeholder="ÖRN: ÖZEL KARIŞIM..." className="w-full bg-transparent border-b border-vora-border/20 py-3 outline-none text-lg font-bold tracking-widest text-vora-primary focus:border-vora-accent transition-all uppercase placeholder:opacity-5" />
                </div>
                <div className="relative group">
                  <label className="text-[9px] font-black text-vora-tertiary uppercase tracking-[0.4em] mb-2 block opacity-30 group-focus-within:text-vora-accent transition-colors">Marka (Opsiyonel)</label>
                  <input type="text" value={form.brand} onChange={(e) => setForm({...form, brand: e.target.value.toUpperCase()})} onKeyDown={(e) => e.key === 'Enter' && handleNextStep()} placeholder="ÖRN: EV YAPIMI..." className="w-full bg-transparent border-b border-vora-border/20 py-3 outline-none text-xs font-bold tracking-widest text-vora-primary focus:border-vora-accent transition-all uppercase placeholder:opacity-5" />
                </div>
              </div>
            </motion.div>
          ) : step === 2 ? (
            <motion.div key="s2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid grid-cols-2 gap-x-10 gap-y-8">
              {[
                ...
                <div key={input.field} className="relative group">
                  <label className="text-[8px] font-black text-vora-tertiary uppercase tracking-[0.3em] mb-1 block opacity-40">{input.label}</label>
                  <input autoFocus={input.field === 'calories'} type="text" inputMode="numeric" value={(form as any)[input.field]} onChange={(e) => handleNumericInput(input.field, e.target.value, input.max)} onKeyDown={(e) => e.key === 'Enter' && handleNextStep()} placeholder="0" className={`w-full bg-transparent border-b border-vora-border/20 py-2 outline-none text-xl font-black tracking-tighter ${input.color} focus:border-vora-accent transition-all placeholder:opacity-5`} />
                </div>
              ))}
            </motion.div>
          ) : (
            <motion.div key="s3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
              <div className="relative group max-w-[180px] mx-auto text-center">
                <label className="text-[9px] font-black text-vora-tertiary uppercase tracking-[0.4em] mb-2 block opacity-30">MİKTAR (G/ML)</label>
                <input autoFocus type="text" inputMode="numeric" value={form.amount} onChange={(e) => handleNumericInput('amount', e.target.value, 5)} onKeyDown={(e) => e.key === 'Enter' && handleFinalSubmit()} className="w-full bg-transparent text-center outline-none text-5xl font-black tracking-tighter text-vora-primary" />
                <div className="h-0.5 w-full bg-vora-accent/20 mt-2" />
              </div>
              <div className="grid grid-cols-4 gap-2">
                {["BREAKFAST", "LUNCH", "DINNER", "SNACK"].map(t => (
                  <button key={t} onClick={() => setForm({...form, type: t as any})} className={`py-3 rounded-xl text-[8px] font-black uppercase tracking-widest transition-all border outline-none ${form.type === t ? "bg-vora-accent text-vora-on-accent border-vora-accent" : "bg-white/[0.02] border-vora-border/10 text-vora-tertiary"}`}>
